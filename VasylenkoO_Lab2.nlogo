...
to draw-maze
  ask turtles [
    ; якщо ми в режимі блукання

    ifelse (hunting = 0) [
      let target-patch 0

      ; відмічаємо поточний патч та шукаємо сусідній чорний
      ask patch-here [
        set pcolor white
        set target-patch one-of neighbours-black with [pcolor = black]
      ]

      ; якщо такого патчу немає - переходимо в режим полювання
      ifelse (target-patch = nobody) [
        set hunting 1
      ][

        ; якщо такий патч є - йдемо до нього та ламаємо стінку (фарбуємо у білий)
      ...
      ifelse (empty? target-patches) [
        ...
        if (target-patch != nobody)[
          ...
          ; якщо такий сусід є - переміщаємо на нього черепашку
          set xcor [pxcor] of p
          set ycor [pycor] of p

          ; ламаємо стінки між білим сусідом та обраним чорним патчем
          set heading towards target-patch
          fd 1
          set pcolor white
