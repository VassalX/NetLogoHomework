...
to setup
  ...
  set hidden-strategy one-of [
    ...
    "tit-for-two-tats" "unforgiving" "custom-strategy" "soft-gruder"
  ]
  reset-ticks
...
to soft-gruder
  if soft-gruder-state > 4 [
    set soft-gruder-state -1
  ]
  ifelse soft-gruder-state = -1 [
    ifelse partner-defected? [
      set soft-gruder-state 0
      set defect-now? item 0 soft-gruder-seq
    ][
      set defect-now? false
    ]
  ] [
    set soft-gruder-state soft-gruder-state + 1
    set defect-now? item soft-gruder-state soft-gruder-seq
  ]
end
...
human-strategy
human-strategy
"act-randomly" "cooperate" "defect" "tit-for-tat" "tit-for-two-tats" "unforgiving" "custom-strategy" "soft-gruder"
7
